...
bool readResistors(int half_gisteresis){
  int val =0;
  int val_big =0; //полученное значение но увеличенное на 10 (val+10)
  int val_small =0; //полученное значение но уменьшенное на 10 (val-10)
  bool changed = false;
  for (byte i = 0; i < channels_count; i++) {
    if(channels[i][c_en] == m_en){
        val = getValueById(i);
        val_small = val - half_gisteresis;
        val_big = val + half_gisteresis;
        if(channels[i][c_speed] < val_small){
           channels[i][c_speed] = val;
          changed = true;
          }else if (channels[i][c_speed] > val_big)
          {
           channels[i][c_speed] = val;
          changed = true;
          }
